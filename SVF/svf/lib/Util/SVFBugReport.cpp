...
cJSON * UseAfterFreeBug::getBugDescription() const
{
    ...
    {
        cJSON *freeLoc = cJSON_Parse(bugEventStack.front().getEventLoc().c_str());
        if (freeLoc == nullptr)
            freeLoc = cJSON_CreateObject();
        cJSON_AddItemToObject(bugDescription, "FreeLocation", freeLoc);
    }
    return bugDescription;
}
...
{
    SVFUtil::errs() << SVFUtil::bugMsg2("\t Use After Free :")
                    << " memory access at : (" << GenericBug::getLoc() << ")\n";
    if (bugEventStack.size() > 1)
    {
        SVFUtil::errs() << "\t\t freed at : ("
                        << bugEventStack.front().getEventLoc() << ")\n";
    }
    SVFUtil::errs() << "\n";
}
