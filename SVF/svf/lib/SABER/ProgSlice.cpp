...
bool ProgSlice::AllPathReachableSolve()
{
    ...
    while(!worklist.empty())
    {
        ...
        // 新增：记录当前节点的路径条件（遍历过程中收集）
        const ICFGNode* nodeICFG = node->getICFGNode();
        std::string nodeLoc = nodeICFG ? nodeICFG->getSourceLoc() : "Unknown Location";
        std::string nodeCondStr = dumpCond(cond);
        // 标记是否为 sink 节点
        bool isSink = (sinks.find(node) != sinks.end());
        // 标记是否为源节点
        bool isSource = (node == source);
        
        // 拼接当前节点的信息行
        std::ostringstream nodeInfo;
        nodeInfo << "Node ID: " << node->getId() << "\n";
        nodeInfo << "  Location: " << nodeLoc << "\n";
        nodeInfo << "  Is Source: " << (isSource ? "Yes" : "No") << "\n";
        nodeInfo << "  Is Sink: " << (isSink ? "Yes" : "No") << "\n";
        nodeInfo << "  In Backward Slice: " << (inBackwardSlice(node) ? "Yes" : "No") << "\n";
        nodeInfo << "  Path Condition: " << nodeCondStr << "\n";
        nodeInfo << "----------------------------------------\n";
        allNodeCondLines.push_back(nodeInfo.str());
    ...
    /*
    // ========== 2. 写入汇总文件（核心：所有节点约束到一个文件） ==========
    // 生成唯一文件名（切片ID + 时间戳，避免覆盖）
    static int sliceId = 0;
    std::time_t now = std::time(nullptr);
    std::string timeStr = std::to_string(now);
    std::string fileName = "slice_" + std::to_string(sliceId++) + "_all_nodes_cond_" + timeStr + ".txt";
    
    std::ofstream outFile(fileName);
    if (outFile.is_open()) {
        // 写入所有节点信息
        for (const auto& line : allNodeCondLines) {
            outFile << line;
        }
        // 追加 sink 节点汇总（方便快速定位核心节点）
        outFile << "\n===== Sink Nodes Summary =====\n";
        int sinkIndex = 0;
        for (const SVFGNode* sink : sinks) {
            //SVF::ProgSlice::Condition sinkCond = getVFCond(sink);
            std::string sinkCondStr = dumpCond(getVFCond(sink));
            const ICFGNode* sinkICFG = sink->getICFGNode();
            std::string sinkLoc = sinkICFG ? sinkICFG->getSourceLoc() : "Unknown Location";
            outFile << "Sink " << sinkIndex++ << ": ID=" << sink->getId() 
                    << ", Location=" << sinkLoc 
                    << ", Condition=" << sinkCondStr << "\n";
        }
        outFile.close();
        outs() << "All nodes' path conditions saved to: " << fileName << "\n";
    } else {
        outs() << "Error: Failed to open file " << fileName << " for writing!\n";
    }
    */
