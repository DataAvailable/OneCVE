...
// SVF/svf/lib/SABER/SrcSnkDDA.cpp 现有头文件后追加
#include "SABER/SaberCheckerAPI.h"  // 引入CheckerAPI定义
#include "Graphs/ICFG.h"            // 引入ICFG定义
#include "Util/Casting.h"           // 引入dyn_cast等类型转换工具


// 在 SrcSnkDDA.cpp 的 using namespace SVF; 之后、SrcSnkDDA::initialize() 之前添加
...
void SrcSnkDDA::initialize()
{
    ...
    SaberCondAllocator* allocator = getSaberCondAllocator();

     allocator->setCallGraph(callgraph);   // 新增
    allocator->setVFG(svfg);
    allocator->setPointerAnalysis(ander);
      // 构建存储映射
    allocator->buildStoreMap();
