...
u32_t AbstractState::hash() const
{
    ...
    size_t h3 = _freedAddrs.size() * 2;
    for (const auto addr : _freedAddrs)
    {
        h3 ^= hf(addr) + 0x9e3779b9 + (h3 << 6) + (h3 >> 2);
    }
    return pairH({pairH({h, h2}), h3});
}
...
AddressValue AbstractState::getGepObjAddrs(u32_t pointer, IntervalValue offset)
{
    ...
    for (APOffset i = lb; i <= ub; i++)
    {
        ...
        {
            // NPD gives explicit NULL and unknown external pointers dedicated
            // sentinel addresses. Preserve those sentinels through a GEP
            // instead of treating them as ordinary PAG objects. This lets the
            // detector report NullMem and suppress InvalidMem without an
            // assertion or a lossy bottom value. Other analyses retain the
            // original GEP behavior.
            if (Options::NPDCheck() &&
                    (isNullMem(addr) || isInvalidMem(addr)))
            {
                gepAddrs.insert(addr);
                continue;
            }
