    ...
    else
    {
        for (const auto& stmt: node->getSVFStmts())
        {
            if (const GepStmt* gep = SVFUtil::dyn_cast<GepStmt>(stmt))
            {
                ...
                GenericBug::BugType bugType;
                if (classifyNullDeref(as, rhs, bugType))
                {
                    AEException bug(stmt->toString());
                    addBugToReporter(bug, stmt->getICFGNode(), bugType);
                }
            }
            else if (const LoadStmt* load = SVFUtil::dyn_cast<LoadStmt>(stmt))
            {
                // like llvm bitcode `p = load q`
                // q is the address being dereferenced; in LoadStmt it is the RHS/source.
...
void NullptrDerefDetector::detectExtAPI(AbstractState& as, const CallICFGNode* call)
{
    ...
    for (const auto &arg: tmp_args)
    {
        ...
        GenericBug::BugType bugType;
        if (argVal && classifyNullDeref(as, argVal, bugType))
        {
            AEException bug(call->toString());
            addBugToReporter(bug, call, bugType);
        }
    }
}


static bool derivesFromInvalidMemory(
    AbstractState& as, const SVFVar* value, Set<NodeID>& visited)
{
    if (value == nullptr || !visited.insert(value->getId()).second)
        return false;

    for (const SVFStmt* incoming : value->getInEdges())
    {
        const SVFVar* source = nullptr;
        if (const LoadStmt* load = SVFUtil::dyn_cast<LoadStmt>(incoming))
        {
            source = load->getRHSVar();
            const AbstractValue sourceValue = as[source->getId()];
            if (sourceValue.isAddr() &&
                    sourceValue.getAddrs().contains(InvalidMemAddr))
                return true;
        }
        else if (const GepStmt* gep = SVFUtil::dyn_cast<GepStmt>(incoming))
            source = gep->getRHSVar();
        else if (const CopyStmt* copy = SVFUtil::dyn_cast<CopyStmt>(incoming))
            source = copy->getRHSVar();
        else if (const PhiStmt* phi = SVFUtil::dyn_cast<PhiStmt>(incoming))
        {
            for (u32_t index = 0; index < phi->getOpVarNum(); ++index)
            {
                if (derivesFromInvalidMemory(
                            as, phi->getOpVar(index), visited))
                    return true;
            }
        }
        else if (const SelectStmt* select =
                     SVFUtil::dyn_cast<SelectStmt>(incoming))
        {
            if (derivesFromInvalidMemory(
                        as, select->getTrueValue(), visited) ||
                    derivesFromInvalidMemory(
                        as, select->getFalseValue(), visited))
                return true;
        }

        if (source != nullptr &&
                derivesFromInvalidMemory(as, source, visited))
            return true;
    }
    return false;
}

bool NullptrDerefDetector::classifyNullDeref(
    AbstractState& as, const SVFVar* value,
    GenericBug::BugType& bugType)
{
    if (value == nullptr)
        return false;
    ...
    // Bottom means that AE has no usable value for this variable. It is not
    // evidence of NULL: unknown root inputs and unsupported value flows can
    // both reach bottom. Explicit NULL is represented by NullMemAddr while the
    // NPD detector is active, so suppressing bottom removes false positives
    // without hiding a concrete null dereference.
    if (isUninit(AbsVal))
        return false;
    if (!AbsVal.isAddr())
        return false;
    ...
    for (const auto &addr: AbsVal.getAddrs())
    {
        if (AbstractState::isNullMem(addr))
            ...
            // A joined state such as {NULL, object} only proves that the
            // pointer may be NULL. Per-translation-unit analysis lacks enough
            // caller/path context to distinguish a real nullable path from a
            // merge artifact, so reporting it causes widespread false
            // positives. NPD reports only a definitely-NULL address state.
            return false;
    }
    if (!hasNull)
        return false;
    // Loads through InvalidMem read SVF's shared black-hole cell. A NULL
    // stored there by an unrelated path is not evidence that this external
    // object's field is NULL. Suppress only that provenance; concrete NULL
    // assignments and NULL-refined branches are still reported.
    Set<NodeID> visited;
    if (derivesFromInvalidMemory(as, value, visited))
        return false;
    bugType = GenericBug::FULLNULLPTRDEREFERENCE;
    return true;
}
...
{
    GenericBug::BugType bugType;
    return !classifyNullDeref(as, value, bugType);
}

bool UseAfterFreeDetector::pointsToFreedMemory(
    AbstractState& as, const SVFVar* pointer, const CallICFGNode** freeSite) const
{
    if (pointer == nullptr)
        return false;

    const AbstractValue value = as[pointer->getId()];
    if (!value.isAddr())
        return false;

    for (const auto address : value.getAddrs())
    {
        if (!as.isFreedMem(address))
            continue;
        if (freeSite != nullptr)
        {
            const auto found = freeSites.find(address);
            *freeSite = found == freeSites.end() ? nullptr : found->second;
        }
        return true;
    }
    return false;
...
void UseAfterFreeDetector::updateLifetimeState(
    AbstractState& as, const CallICFGNode* call)
{
    ...
    if (checkerAPI->isMemAlloc(function))
    {
        const SVFVar* result = call->getRetICFGNode()->getActualRet();
        if (result != nullptr && as[result->getId()].isAddr())
        {
            for (const auto address : as[result->getId()].getAddrs())
            {
                as.removeFromFreedAddrs(address);
                freeSites.erase(address);
            }
        }
    }

    if (!checkerAPI->isMemDealloc(function))
        return;

    // Match Saber's deallocation modeling: every pointer-valued actual
    // parameter of a configured deallocator is a possible released object.
    for (u32_t index = 0; index < call->arg_size(); ++index)
    {
        const SVFVar* argument = call->getArgument(index);
        if (argument == nullptr || !as[argument->getId()].isAddr())
            continue;
        for (const auto address : as[argument->getId()].getAddrs())
        {
            if (AbstractState::isNullMem(address) || AbstractState::isInvalidMem(address))
                continue;
            as.addToFreedAddrs(address);
            freeSites[address] = call;
        }
    }
}

void UseAfterFreeDetector::detect(AbstractState& as, const ICFGNode* node)
{
    if (const CallICFGNode* call = SVFUtil::dyn_cast<CallICFGNode>(node))
    {
        updateLifetimeState(as, call);
        const FunObjVar* function = call->getCalledFunction();
        if (function != nullptr &&
                SaberCheckerAPI::getCheckerAPI()->isMemDealloc(function))
        {
            // Passing an object to its deallocator is not itself a UAF. A
            // second deallocation remains the responsibility of -dfree.
            return;
        }
        if (function != nullptr && SVFUtil::isExtCall(function))
    ...
    {
        const SVFVar* address = nullptr;
        if (const LoadStmt* load = SVFUtil::dyn_cast<LoadStmt>(statement))
        {
            // q = *p: the RHS/source p is the dereferenced address.
            address = load->getRHSVar();
        }
        else if (const StoreStmt* store = SVFUtil::dyn_cast<StoreStmt>(statement))
        {
            // *p = q: the LHS/destination p is the dereferenced address.
            address = store->getLHSVar();
        }
        else if (const GepStmt* gep = SVFUtil::dyn_cast<GepStmt>(statement))
        {
            address = gep->getRHSVar();
        }
        else
        {
            continue;
        ...
        const CallICFGNode* freeSite = nullptr;
        if (pointsToFreedMemory(as, address, &freeSite))
        {
            AEException bug(statement->toString());
            addBugToReporter(bug, statement->getICFGNode(), freeSite);
        }
    }
}

void UseAfterFreeDetector::detectExtAPI(
    AbstractState& as, const CallICFGNode* call)
{
    std::vector<u32_t> dereferencedArguments;
            ...
            ExtAPI::getExtAPI()->getExtFuncAnnotations(call->getCalledFunction()))
    {
        ...
        {
            if (call->arg_size() < 4)
                dereferencedArguments.insert(dereferencedArguments.end(), {0, 1});
            else
                dereferencedArguments.insert(dereferencedArguments.end(), {1, 2, 3, 4});
        }
        else if (annotation.find("MEMSET") != std::string::npos)
        {
            dereferencedArguments.push_back(0);
        }
        else if (annotation.find("STRCPY") != std::string::npos ||
    ...
    for (const u32_t index : dereferencedArguments)
    {
        if (index >= call->arg_size())
            continue;
        const CallICFGNode* freeSite = nullptr;
        if (pointsToFreedMemory(as, call->getArgument(index), &freeSite))
        {
            AEException bug(call->toString());
            addBugToReporter(bug, call, freeSite);
        }
    }
}

void UseAfterFreeDetector::addBugToReporter(
    const AEException& error, const ICFGNode* access,
    const CallICFGNode* freeSite)
{
    const std::string location = access->getSourceLoc();
    if (!bugLoc.insert(location).second)
        return;

    GenericBug::EventStack eventStack;
    if (freeSite != nullptr)
        eventStack.emplace_back(SVFBugEvent::CallSite, freeSite);
    eventStack.emplace_back(SVFBugEvent::SourceInst, access);
    recorder.addAbsExecBug(GenericBug::USEAFTERFREE, eventStack, 0, 0, 0, 0);
    nodeToBugInfo[access] = error.what();
}
...
void UseAfterFreeDetector::reportBug()
{
    if (nodeToBugInfo.empty())
        return;
    std::cerr << "###################### Use After Free ("
              << nodeToBugInfo.size() << " found)######################\n";
    std::cerr << "---------------------------------------------\n";
    for (const auto& entry : nodeToBugInfo)
    {
