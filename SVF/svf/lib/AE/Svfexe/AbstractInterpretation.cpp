...
void AbstractInterpretation::analyse()
{
    ...
    const CallGraphNode* mainNode = svfir->getCallGraph()->getCallGraphNode("main");
    auto analyseRoot = [this](const FunObjVar* function, bool seedUnknownParameters)
    {
        const auto found = funcToWTO.find(function);
        if (found == funcToWTO.end() || found->second == nullptr)
            return;
        const FunEntryICFGNode* entry = icfg->getFunEntryICFGNode(function);
        abstractTrace[entry] = abstractTrace[icfg->getGlobalICFGNode()];
        if (seedUnknownParameters)
        {
            if (Options::NPDCheck())
            {
                // A source-level compilation unit without main is analyzed
                // without the program's real initialization sequence. Its
                // zero-initialized global pointer fields may therefore be
                // overwritten by another unit before this exported root is
                // ever called. Treat those entry-only NULL values as unknown;
                // explicit NULL assignments executed inside the function are
                // still represented by NullMemAddr and remain detectable.
                std::vector<u32_t> externalGlobalLocations;
                for (const auto& location : abstractTrace[entry].getLocToVal())
                {
                    const AbstractValue& value = location.second;
                    if (!value.isAddr() || value.getAddrs().empty())
                        continue;
                    bool onlyNull = true;
                    for (const auto address : value.getAddrs())
                    {
                        if (!AbstractState::isNullMem(address))
                        {
                            onlyNull = false;
                            break;
                        }
                    ...
                    if (onlyNull)
                        externalGlobalLocations.push_back(location.first);
                }
                for (const auto location : externalGlobalLocations)
                    abstractTrace[entry].store(
                        location, AddressValue(InvalidMemAddr));
            }
            // A standalone library bitcode unit has no caller state for its
            // exported roots.  Leaving pointer formals at bottom makes the
            // NPD detector confuse unknown caller input with a concrete NULL.
            // InvalidMem is SVF's black-hole/unknown address and is therefore
            // the conservative non-null-evidence seed for external inputs.
            for (const SVFVar* parameter : entry->getFormalParms())
            {
                if (parameter != nullptr && parameter->isPointer())
                {
                    abstractTrace[entry][parameter->getId()] =
                        AddressValue(InvalidMemAddr);
                }
            }
        }
        seededEntryNodes.insert(entry);
        const ICFGWTO* wto = found->second;
        handleWTOComponents(wto->getWTOComponents());
    };

    if (mainNode != nullptr)
        analyseRoot(mainNode->getFunction(), false);

    // OneCVE commonly scans one compilation unit per bitcode file. Library
    // units do not have main, so analyze each call-graph root and let normal
    // interprocedural handling cover its callees.
    if (mainNode == nullptr || Options::RunUncallFuncs())
    {
        for (const auto& entry : *svfir->getCallGraph())
        {
            const CallGraphNode* callGraphNode = entry.second;
            const FunObjVar* function = callGraphNode->getFunction();
            if (function->isDeclaration() || callGraphNode == mainNode ||
                    !callGraphNode->getInEdges().empty())
                continue;
            analyseRoot(function, true);
        }
    }
}
...
void AbstractInterpretation::handleGlobalNode()
{
    ...
    // AE traditionally represents both an explicit NULL and an unavailable
    // abstract value as bottom. That ambiguity creates NPD reports for every
    // value the standalone-unit analysis cannot model. Give explicit NULL a
    // dedicated address only for -npd; the other AE detectors keep their
    // original state representation unchanged.
    abstractTrace[node][IRGraph::NullPtr] = Options::NPDCheck()
            ? AbstractValue(AddressValue(NullMemAddr))
            : AbstractValue(AddressValue());
...
bool AbstractInterpretation::mergeStatesFromPredecessors(const ICFGNode * icfgNode)
{
    ...
    if (workList.size() == 0)
    {
        if (seededEntryNodes.erase(icfgNode) != 0)
            return true;
        return false;
    }
    else
    {
        seededEntryNodes.erase(icfgNode);
        ...
        AbstractState& as)
{
    ...
    // Refine pointer states across NULL checks for NPD. The generic AE logic
    // skips address comparisons, which otherwise lets NullMem flow into the
    // body of `if (p)`, `p != NULL` and `while (p)` and creates false reports.
    if (op0 == IRGraph::NullPtr || op1 == IRGraph::NullPtr)
    {
        if (Options::NPDCheck() && op0 != op1)
        {
            const bool isEquality =
                predicate == CmpStmt::Predicate::ICMP_EQ ||
                predicate == CmpStmt::Predicate::FCMP_OEQ ||
                predicate == CmpStmt::Predicate::FCMP_UEQ;
            const bool isInequality =
                predicate == CmpStmt::Predicate::ICMP_NE ||
                predicate == CmpStmt::Predicate::FCMP_ONE ||
                predicate == CmpStmt::Predicate::FCMP_UNE;
            const NodeID pointer = op0 == IRGraph::NullPtr ? op1 : op0;
            if ((isEquality || isInequality) &&
                    new_es.inVarToAddrsTable(pointer))
            {
                const bool equalBranch = isEquality ? succ != 0 : succ == 0;
                const AddressValue current = new_es[pointer].getAddrs();
                AddressValue refined;
                if (equalBranch)
                {
                    // InvalidMem represents an unknown external pointer. On
                    // the equality edge, the condition itself establishes
                    // that its concrete value is NULL.
                    if (current.contains(NullMemAddr) ||
                            current.contains(InvalidMemAddr))
                        refined.insert(NullMemAddr);
                }
                else
                {
...
void AbstractInterpretation::handleSVFStatement(const SVFStmt *stmt)
{
    ...
    else if (const RetPE *retPE = SVFUtil::dyn_cast<RetPE>(stmt))
    {
        ...
        assert(false && "implement this part");
    // NullPtr is index 0 and must retain the representation selected above.
    const AbstractValue& nullValue =
        getAbsStateFromTrace(stmt->getICFGNode())[IRGraph::NullPtr];
    if (Options::NPDCheck())
    {
        bool hasNullAddress = false;
        for (const auto address : nullValue.getAddrs())
            hasNullAddress = hasNullAddress || AbstractState::isNullMem(address);
        assert(nullValue.isAddr() && hasNullAddress &&
               "NPD explicit NULL marker must not be changed");
    }
    else
    {
        assert(!nullValue.isInterval() && !nullValue.isAddr());
    }
}
    ...
    for (auto it = svfir->getICFG()->begin(); it != svfir->getICFG()->end(); ++it)
    {
        ...
        if (const CallICFGNode *call = SVFUtil::dyn_cast<CallICFGNode>(node))
        {
            if (const FunObjVar *fun = call->getCalledFunction())
            {
                ...
                if (Options::NullDerefCheck() || Options::NPDCheck())
                {
