...
public:
    ...
    enum DetectorKind
    {
        ...
        USE_AFTER_FREE, ///< Detector for accesses through freed addresses.
...
public:
    ...
    */
        ...
        GenericBug::BugType bugType)
    {
        ...
        recoder.addAbsExecBug(bugType, eventStack, 0, 0, 0, 0);
        nodeToBugInfo[node] = e.what(); // Record the exception information for the node
    }
    ...
    bool classifyNullDeref(
        AbstractState& as, const SVFVar* ptr,
        GenericBug::BugType& bugType);
...
/**
 * Detects memory reads, writes, pointer arithmetic and modeled external API
 * accesses whose address may already have been released.
 */
...
private:
    bool pointsToFreedMemory(AbstractState& as, const SVFVar* pointer,
                             const CallICFGNode** freeSite = nullptr) const;
    void updateLifetimeState(AbstractState& as, const CallICFGNode* call);
    void detectExtAPI(AbstractState& as, const CallICFGNode* call);
    void addBugToReporter(const AEException& error, const ICFGNode* access,
                          const CallICFGNode* freeSite);

    Set<std::string> bugLoc;
    SVFBugReport recorder;
    Map<const ICFGNode*, std::string> nodeToBugInfo;
    Map<NodeID, const CallICFGNode*> freeSites;
};
}
