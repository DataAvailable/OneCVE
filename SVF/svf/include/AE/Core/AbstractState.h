...
public:
    ...
    /// move constructor
    AbstractState(AbstractState&&rhs) : _freedAddrs(std::move(rhs._freedAddrs)),
...
public:
    ...
    void removeFromFreedAddrs(NodeID addr)
    {
        _freedAddrs.erase(addr);
    }
...
public:
    ...
    bool operator>=(const AbstractState&rhs) const
    {
        return geqVarToValMap(_varToAbsVal, rhs.getVarToVal()) &&
               ...
               std::includes(_freedAddrs.begin(), _freedAddrs.end(),
                             rhs._freedAddrs.begin(), rhs._freedAddrs.end());
    }
