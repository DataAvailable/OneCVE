...
public:
    enum BugType {FULLBUFOVERFLOW, PARTIALBUFOVERFLOW, NEVERFREE, PARTIALLEAK, DOUBLEFREE, USEAFTERFREE, FILENEVERCLOSE, FILEPARTIALCLOSE, FULLNULLPTRDEREFERENCE, PARTIALNULLPTRDEREFERENCE};
...
public:
                       ...
                       s64_t allocLowerBound, s64_t allocUpperBound, s64_t accessLowerBound, s64_t accessUpperBound)
    {
        ...
        // Keep NPD findings machine-discoverable in the same stderr stream as
        // Saber findings. Preserve the existing terminal behavior of the
        // unrelated abstract-execution detectors.
        if (bugType == GenericBug::FULLNULLPTRDEREFERENCE ||
                bugType == GenericBug::PARTIALNULLPTRDEREFERENCE ||
                bugType == GenericBug::USEAFTERFREE)
        {
            newBug->printBugToTerminal();
        }
    }
