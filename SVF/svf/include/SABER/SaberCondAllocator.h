...
// SaberCondAllocator.h 头部补充
#include "SVFIR/SVFVariables.h"  // 包含 ConstIntValVar/ConstNullPtrValVar/ValVar 等定义
#include "SVFIR/SVFStatements.h"  // 包含 CmpStmt 定义（如果未引入）
#include "Graphs/VFG.h"
#include "MemoryModel/PointerAnalysis.h"
#include "MSSA/MSSAMuChi.h"
#include "MSSA/MemRegion.h"
#include "MSSA/MemSSA.h"   // 包含 MemSSA 头文件
...
public:
    ...
    const VFG* vfg;
    PointerAnalysis* pta;
    void setVFG(const VFG* v) { vfg = v; }
    void setPointerAnalysis(PointerAnalysis* p) { pta = p; }

   CallGraph* callGraph;   // 新增
   void setCallGraph(CallGraph* cg) { callGraph = cg; }

struct StoreInfo {
    const StoreStmt* store;
    NodeID objId;
    size_t idx;  // 指令在基本块中的顺序（索引）
};
Map<const SVFBasicBlock*, std::vector<StoreInfo>> bbStores;  // 按基本块存储所有写入

Map<NodeID, Set<const FunObjVar*>> globalToAssigningFuncs; // 记录哪些函数对全局变量有赋值

void buildStoreMap();  // 构建映射


bool findLatestStore(const SVFBasicBlock* loadBB, NodeID objId, const StoreStmt*& latestStore,
                     std::set<const SVFBasicBlock*>& visited, const ICFGNode* loadNode = nullptr);
...
private:
    ...
    /// 自定义判断是否为左右变量和常量
    bool extractVarAndConstant(const CmpStmt* cmp, const ValVar*& var, s64_t& constant) ;
    // 1. 声明 evaluateConstantCmpBranch（参数+返回值必须和.cpp一致）
    Condition evaluateConstantCmpBranch(const BranchStmt *branchStmt, const SVFBasicBlock* succ);


    bool evaluateCmp(u32_t predicate, s64_t lhs, s64_t rhs) const;
    static const u32_t MAX_DEPTH = 30;

    const CallICFGNode* findCallSiteFromNode(const SVFGNode* node) const;
    // 修改签名
    bool getConstantValue(const SVFVar* var, s64_t& outVal, u32_t depth = 0);

    

     const CallICFGNode* currentCallSite = nullptr;
    void setCurrentCallSite(const CallICFGNode* cs) { currentCallSite = cs; }
