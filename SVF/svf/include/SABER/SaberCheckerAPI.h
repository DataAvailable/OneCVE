...
public:
    ...
    inline const TDAPIMap& getTDAPIMap() const {
        return tdAPIMap;
    }

  /// 从配置文件加载自定义 API（格式：类型: 函数名列表）
    bool loadCustomAPIsFromFile(const std::string& filename);

    /// 直接添加一个自定义 API
    void addCustomAPI(const std::string& funcName, CHECKER_TYPE type);
